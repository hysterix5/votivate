...
class ActivePollPage extends StatefulWidget {
  final DocumentSnapshot optionData;
  const ActivePollPage(this.optionData, {Key? key}) : super(key: key);

  @override
  State<ActivePollPage> createState() => _ActivePollPageState();
}

class _ActivePollPageState extends State<ActivePollPage> {
  ...
  void initState() {
    ...
    title = widget.optionData['name'] ?? 'Default Title';
      ...
      if (electionDoc.exists) {
        ...
        if (participants != null && participants.isNotEmpty) {
          for (dynamic participant in participants) {
            // Assuming participant contains a 'name' field
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          title,
          style: const TextStyle(color: Color.fromARGB(255, 238, 240, 237)),
        ),
